...
// TODO: delete
func TestXMLEventDescriptors(t *testing.T) {
	// Create consumer for Microsoft-Windows-PowerShell provider (manifest-based)
	c := NewConsumer(context.Background())
	defer c.Stop()

	c.EventCallback = nil

	// Add callback to compare descriptors
	c.EventRecordHelperCallback = func(h *EventRecordHelper) error {
		if !h.TraceInfo.IsXML() {
			return nil
		}

		// Compare descriptors
		erDesc := h.EventRec.EventHeader.EventDescriptor
		tiDesc := h.TraceInfo.EventDescriptor

		// Deep compare descriptors
		if erDesc.Id != tiDesc.Id ||
			erDesc.Version != tiDesc.Version ||
			erDesc.Channel != tiDesc.Channel ||
			erDesc.Level != tiDesc.Level ||
			erDesc.Opcode != tiDesc.Opcode ||
			erDesc.Task != tiDesc.Task ||
			erDesc.Keyword != tiDesc.Keyword {

			t.Errorf("Descriptors differ for provider %s event %d",
				h.TraceInfo.ProviderName(), h.TraceInfo.EventID())
		}
		return nil
	}

	// Enable PowerShell provider
	session := NewRealTimeSession("TestingGoEtw")
	defer session.Stop()

	// Add more manifest providers
	providers := []string{
	...
	for _, p := range providers {
		err := session.EnableProvider(MustParseProvider(p))
		if err != nil {
			t.Fatal(err)
		}
	}

	c.FromTraceNames("EventLog-Application")
