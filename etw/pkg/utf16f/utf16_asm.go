...
func DecodeSIMD_v3(s []uint16) string {
	...
	written := utf16ToStringSSE2_v3(&s[0], len(s), buf)
...
func DecodeSIMD_v4(s []uint16) string {
	// Pre-calculate max size
	maxLen := 0
	for i, v := range s {
		if v == 0 {
	...
	// Single allocation for buffer
	buf := make([]byte, maxLen)
	written := utf16ToStringSSE2_v4(&s[0], len(s), buf)
